...
class HierarchicalMatrixBase;
/// inherit the traits from the Eigen::SparseMatrix class
namespace internal {
template <typename Derived>
struct traits<HierarchicalMatrixBase<Derived>>
    : public internal::traits<
          SparseMatrix<typename HierarchicalMatrixTraits<Derived>::Scalar>> {};
}  // namespace internal

// actual definition of the class
template <typename Derived>
class HierarchicalMatrixB : public EigenBase<HierarchicalMatrix<Derived>> {
 public:
  ...
  /*
  * \brief Computes a dense matrix representation of the hierarchical
* matrix
*        by multiplying it with a unit matrix.
*
* \attention This function may be very slow and memory intensive for large
*            matrices.
  */
  Eigen::Matrix<typename Bembel::PDEproblemTraits<Derived>::Scalar,
  ...
  getDense() const {
    ...
    Eigen::Matrix<typename Bembel::PDEproblemTraits<Derived>::Scalar,
        ...
        dense_matrix =
            Eigen::Matrix<typename Bembel::PDEproblemTraits<Derived>::Scalar,
                          Eigen::Dynamic, Eigen::Dynamic>::Zero(rows, cols);
    for (int coli = 0; coli < cols; ++coli) {
      unit_vector.setZero();
      unit_vector(coli) = 1.;
      dense_matrix.col(coli) = (*this) * unit_vector;
    }
    return dense_matrix;
  }
...
struct generic_product_impl<HierarchicalMatrix<Derived>, Rhs, SparseShape,
  ...
  static void scaleAndAddTo(Dest& dst, const HierarchicalMatrix<Derived>& lhs,
    ...
    // Here we could simply call dst.noalias() += lhs.my_matrix() *
    // rhs,
