...
int main() {
  ...
  // pointers
  readTXT("data/vertices_square250k.txt", P_sources, 2);
  readTXT("data/quadrature3_points_square250k.txt", P_quad, 2);
  readTXT("data/quadrature3_weights_square250k.txt", w_vec);
  readTXT("data/quadrature_border250k.txt", P_quad_border, 2);
  readTXT("data/weights_border250k.txt", w_vec_border);
  readTXT("data/normals250k.txt", Normals, 2);
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  // U_BC vector
  ...
  std::vector<FMCA::Scalar> sigmas = {2.1*sigma_h, 2.5*sigma_h};
  // {1.5*sigma_h, 1.75*sigma_h, 2*sigma_h, 2.25*sigma_h};
  ...
  for (FMCA::Scalar sigma : sigmas) {
    ...
    // plot solution 
    {
      ...
      FMCA::Grid2D grid(min, max, 500, 500);
      ...
      grid.plotFunction("square_Halton100k.vtk", rec);
    }
    ...
    // plot error 
    {
      ...
      FMCA::Vector srec = hmat.action(mat_eval, sx);
      FMCA::Vector rec = hct_eval.toNaturalOrder(srec);
      grid.plotFunction("square_Halton_error100k.vtk", rec);
    }
