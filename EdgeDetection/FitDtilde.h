...
std::map<const TreeType*, FitResult<Scalar>> FitDtilde(
    ...
    for (const auto& [leaf, dataPair] : leafData) {
        ...
        Scalar norm = std::sqrt(std::accumulate(coefficients.begin(), coefficients.end(), Scalar(0), 
        ...
        if (std::all_of(coefficients.begin(), coefficients.end(),
                        [&](Scalar c){ return std::abs(c) < FMCA_ZERO_TOLERANCE; })) {
            results[leaf] = FitResult<Scalar>(dtilde, 0);
            continue;
        }
        ...
        if (n >= 2 && 
            std::abs(coefficients[n - 1] / norm) < smallThreshold && 
            std::abs(coefficients[n - 2] / norm) < smallThreshold) {
            results[leaf] = FitResult<Scalar>(dtilde, 0);
            continue;
        }
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            b(i) = (coefficients[i] == 0) ? FMCA_ZERO_TOLERANCE : std::log(std::abs(coefficients[i]));
        }

        // Compute the least squares solution using QR decomposition
        Eigen::Matrix<Scalar, 2, 1> x = A.colPivHouseholderQr().solve(b);
