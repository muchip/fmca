...
struct compute_variable_order_cluster_bases_impl {
  ...
  static void compute(TreeBase<Derived> &CT, const Moments &mom) {
    ...
    if (H2T.nSons()) {
      ...
      for (auto i = 0; i < H2T.nSons(); ++i) {
        const Matrix &Xi_dad = mom.interp(H2T.level()).Xi();
        const Matrix &Xi_son = mom.interp(H2T.sons(i).level()).Xi();
        Matrix E(Xi_dad.cols(), Xi_son.cols());
        for (auto j = 0; j < E.cols(); ++j)
          E.col(j) = mom.interp(H2T.level())
                         .evalPolynomials((Xi_son.col(j).array() *
  ...
  static int check_transfer_matrices(TreeBase<Derived> &CT,
    ...
    if (H2T.nSons()) {
      ...
      for (auto i = 0; i < H2T.nSons(); ++i) {
        H2T.V().conservativeResize(H2T.Es()[i].rows(),
      ...
      {
        std::cout << V - H2T.V() << std::endl;
        std::cout << nrm << std::endl;
        std::cout << H2T.level() << std::endl;
      }
