...
//         return r < FMCA_ZERO_TOLERANCE ? 0 : log(r / l_) * (r / l_) * (r /
//         l_);
//       };
...
//       kernel_ = [this](Scalar r) {
//         return (1. + std::sqrt(3) / l_ * r) * std::exp(-std::sqrt(3) * r /
//         l_);
//       };
//     ////////////////////////////////////////////////////////////////////////////
...
//     // else if (ktype_ == "GAUSSIAN_LL")
//     //   kernel_ = [this](Scalar r) {
//     //     return  4 / pow(l_,4) * (3 - 3 * r * r / (l_, l_) + pow(r,4) / (4
//     * pow(l_,4))) * exp(-0.5 * r * r / (l_ * l_)) ;
//     //   };
//     else
...
class CovarianceKernel {
 public:
  ...
  CovarianceKernel(CovarianceKernel &&other) noexcept {
  ...
  CovarianceKernel(const std::string &ktype, Scalar l = 1., Scalar c = 1.,
    ...
    if (ktype_ == "BIHARMONIC2D")
      kernel_ = [this](Scalar r, Scalar, Scalar) {
        return r < FMCA_ZERO_TOLERANCE ? 0 : log(r / l_) * (r / l_) * (r / l_);
      };
    else if (ktype_ == "BIHARMONIC3D")
      kernel_ = [this](Scalar r, Scalar, Scalar) { return r / l_; };
    else if (ktype_ == "TRIHARMONIC3D")
      kernel_ = [this](Scalar r, Scalar, Scalar) {
        return (r / l_) * (r / l_) * (r / l_);
      };
    else if (ktype_ == "EXPONENTIAL")
      kernel_ = [this](Scalar r, Scalar, Scalar) { return std::exp(-r / l_); };
    else if (ktype_ == "MATERN32")
      kernel_ = [this](Scalar r, Scalar, Scalar) {
      ...
      kernel_ = [this](Scalar r, Scalar x_norm, Scalar y_norm) {
        return 0.5 * (std::sqrt((r / l_) * (r / l_) + c_ * c_) -
               std::sqrt((x_norm / l_) * (x_norm / l_) + c_ * c_) -
               std::sqrt((y_norm / l_) * (y_norm / l_) + c_ * c_) -
               std::sqrt(c_ * c_));
      };
    else if (ktype_ == "RADIAL")
      kernel_ = [this](Scalar r, Scalar x_norm, Scalar y_norm) {
        return 0.5 * (r - x_norm - y_norm);
      };
    else
      assert(false && "desired kernel not implemented");
  }
  ...
  Scalar operator()(const Eigen::MatrixBase<Derived> &x,
                    const Eigen::MatrixBase<OtherDerived> &y) const {
    return kernel_((x - y).norm(), x.norm(), y.norm());
  }
 ...
 private:
  std::function<Scalar(Scalar, Scalar, Scalar)> kernel_;
  std::string ktype_;
  Scalar l_;
  Scalar c_;
  Scalar nu_;
...
#endif  // FMCA_COVARIANCEKERNEL_COVARIANCEKERNEL_H_
