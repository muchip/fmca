...
// Component d of a kernel derivative.  The lambda receives (x_d, y_d, r, |x|),
// where r = ||x - y||, and returns the requested derivative component.
class GradKernel {
 public:
  GradKernel() {};
  ...
  GradKernel(const std::string& ktype, Scalar l = 1., Scalar c = 1., int d = 0)
    ...
    ////////////////////////////////////////////////////////////////////////////
    if (ktype_ == "MATERN32_FIRST_DERIVATIVE")
      gradkernel_ = [this](Scalar x, Scalar y, Scalar r, Scalar) {
        return -3.0 / (l_ * l_) * std::exp(-std::sqrt(3.0) * r / l_) * (x - y);
      };
...
#endif  // FMCA_COVARIANCEKERNEL_GRADKERNEL_H_
