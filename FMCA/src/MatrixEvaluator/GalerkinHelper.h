...
double analyticIntFD(double s, double alpha, double stau, double sx, double tx,
  ...
  const double txmalphasx = tx - alphasx;
  ...
  const double v = (secrtxpr - q) / (sqrtonepalpha2 * smp);
  ...
  const double firstexp =
      -ux * alpha / sqrtonepalpha2 * log(sqrtonepalpha2 * smp + secrtxpr);
  const double I1 = -0.5 * ux - log(v * v + A1 * v + B1) +
                    (stau - sx) * ux / abs(ux) * atan(0.5 * (2 * v + A1) / G1);
  const double I2 = 0.5 * ux - log(v * v + A2 * v + B2) -
                    (stau - sx) * ux / abs(ux) * atan(0.5 * (2 * v + A2) / G2);
  return 3 * (firstexp + I1 + I2);
}
