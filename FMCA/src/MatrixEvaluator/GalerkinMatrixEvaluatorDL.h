    ...
    for (auto j = 0; j < TC.indices().size(); ++j) {
      ...
      for (auto i = 0; i < TR.indices().size(); ++i) {
        ...
        if (TC.indices()[j] != TR.indices()[i]) {
          value_type nom = (el1.mp_ - el2.mp_).dot(el2.cs_.col(2));
          value_type r = std::pow((el1.mp_ - el2.mp_).norm(), 3.);
          (*retval)(i, j) =
          ...
          if (is_admissible(el1, el2)) {
            ...
            for (auto k = 0; k < Rq_.xi.cols(); ++k) {
              ...
              val += Rq_.w(k) * analyticIntS(el2, qp);
            }
            val *= 2 * cnst * sqrt(el2.volel_) / sqrt(el1.volel_);
            std::cout << nom << " " << r << "\n";
            std::cout << (*retval)(i, j) << " " << val << " "
                      << ((*retval)(i, j) - val) / val << std::endl;
          }
        } else {
          ...
          for (auto k = 0; k < Rq_.xi.cols(); ++k) {
            ...
            (*retval)(i, j) += Rq_.w(k) * analyticIntS(el2, qp);
          }
