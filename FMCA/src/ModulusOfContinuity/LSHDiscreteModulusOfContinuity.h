...
#pragma omp parallel for reduction(max : max_quotient)
    ...
    for (Index k = 1; k <= K_; ++k) {
      ...
      XNk_indices_[k] =
          greedySetCoveringLSH(lsh_, Pprev, P, XNk_indices_[k - 1], Rkr);
...
#pragma omp parallel for reduction(max : max_quotient)
      for (Index i = 0; i < Ploc.cols(); ++i) {
        std::vector<Index> lsh_nn_idcs = lsh_.computeAENN(P, Ploc.col(i), Rkr);

        std::vector<Index> nn_idcs = globalLocalMapping(
            lsh_nn_idcs, XNk_indices_[k]); // filters neighbours that are in the
                                           // current covering
...
#pragma omp parallel for reduction(max : max_quotient)
    for (Index i = 0; i < Ploc.cols(); ++i) {
      ...
      std::vector<Index> lsh_nn_idcs =
          lsh_.computeAENN(P, Ploc.col(i), tgrid_[k]);

      std::vector<Index> nn_idcs =
          globalLocalMapping(lsh_nn_idcs, XNk_indices_[k]);
