...
#pragma omp atomic capture
        ...
        while (i < map_size) {
          ...
          if (pc->nSons()) {
            Index cols = 0;
            Index rows = pr->Q().cols();
            for (auto k = 0; k < pc->nSons(); ++k)
              cols += pc->sons(k).nscalfs();
            ...
            for (auto k = 0; k < pc->nSons(); ++k) {
              ...
              if (it3 != pattern_[son_lvl].end()) {
                block.middleCols(cur_size, nscalfs) =
                    (it3->second).leftCols(nscalfs);
              } else {
                block.middleCols(cur_size, nscalfs) =
            ...
            if (!pr->nSons()) {
              ...
              Index cols = 0;
              Index rows = pc->Q().cols();
              for (auto k = 0; k < pr->nSons(); ++k)
                cols += pr->sons(k).nscalfs();
              block.resize(rows, cols);
              Index cur_size = 0;
              for (auto k = 0; k < pr->nSons(); ++k) {
                ...
                if (it3 != pattern_[son_lvl].end()) {
                  block.middleCols(cur_size, nscalfs) =
                      (it3->second).transpose().leftCols(nscalfs);
                } else {
                  block.middleCols(cur_size, nscalfs) =
    ...
    if (compareCluster(TR, TC, eta_) == LowRank) {
      ...
      switch (the_case) {
        ...
        case 2: {
          ...
          Index cols = 0;
          Index rows = TR.Q().cols();
          for (auto k = 0; k < TC.nSons(); ++k) cols += TC.sons(k).nscalfs();
          buf.resize(rows, cols);
          Index cur_size = 0;
          for (auto j = 0; j < TC.nSons(); ++j) {
            const Index nscalfs = TC.sons(j).nscalfs();
            buf.middleCols(cur_size, nscalfs) =
                recursivelyComputeBlock(TR, TC.sons(j), e_gen)
                    .leftCols(nscalfs);
            cur_size += nscalfs;
          }
          return buf * TC.Q();
        ...
        case 1: {
          ...
          Index cols = 0;
          Index rows = TC.Q().cols();
          for (auto k = 0; k < TR.nSons(); ++k) cols += TR.sons(k).nscalfs();
          buf.resize(rows, cols);
          Index cur_size = 0;
          for (auto i = 0; i < TR.nSons(); ++i) {
            const Index nscalfs = TR.sons(i).nscalfs();
            buf.middleCols(cur_size, nscalfs) =
                recursivelyComputeBlock(TR.sons(i), TC, e_gen)
                    .transpose()
                    .leftCols(nscalfs);
            cur_size += nscalfs;
          }
