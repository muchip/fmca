...
#if 0
      if (j > 2) {
        const auto &idx = m_blx_.idx()[n_clusters_ - j + 2];
        std::vector<Matrix> &val = m_blx_.val()[n_clusters_ - j + 2];
        const Derived *pc = s_mapper_[n_clusters_ - j + 2];
        for (int k = 0; k < idx.size(); ++k) {
          const Derived *pr = s_mapper_[idx[k]];
          if (!pr->is_root() && !pc->is_root()) {
            std::cout << val[k].rows() << " " << val[k].cols() << " "
                      << pr->nsamplets() << " " << pc->nsamplets() << std::endl;
            val[k] = val[k].bottomRightCorner(pr->nsamplets(), pc->nsamplets());
          }
        }
      }
#endif
