...
struct SampletTreeBase : public ClusterTreeBase<Derived> {
  ...
  Matrix sampletTransform(const Matrix &data) const {
    ...
    Derived *curr_cluster = this;
    Matrix retval(data.rows(), data.cols());
    retval.setZero();
    // create a loop that resembles DFS
    do {
      // if a cluster has children, add them to the stack
      if (curr_cluster->nSons()) {
        for (auto i = 0; i < curr_cluster->nSons(); ++i)
          stack.push_back(std::addressof(curr_cluster->sons(i)));
        curr_cluster = stack.back();
        stack.pop_back();
      } else {
  ...
  private:
    ...
    void inverseSampletTransformRecursion(const Matrix &data, Matrix *fvec,
                                          Matrix *ddata, Index ddata_offset = 0)
        const {
      Matrix retval;
      if (is_root()) {
        retval = Q().leftCols(nscalfs()) *
                     data.middleRows(start_index(), nscalfs()) +
                 Q().rightCols(nsamplets()) *
                     data.middleRows(start_index() + nscalfs(), nsamplets());
      } else {
        retval = Q().leftCols(nscalfs()) *
                 ddata->middleRows(ddata_offset, nscalfs());
        if (nsamplets())
