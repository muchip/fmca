...
class SampletMatrixCompressor {
 public:
  ...
  struct MatrixBlockTask {
    ...
    // Dependency info
    std::vector<MatrixBlockTask*> row_children;
    std::vector<MatrixBlockTask*> col_children;
    ...
    std::atomic<int> row_children_completed{
        0};  // children that have finished computing
    std::atomic<int> col_children_completed{
        0};  // children that have finished computing
    std::atomic<int> parents_notified{
        0};  // parents that have notified us they're done using our block

    // Relationship flags - pre-computed durante init
    bool is_row_child_of_parent1 = false;  // NEW
    bool is_col_child_of_parent1 = false;  // NEW
    bool is_row_child_of_parent2 = false;  // NEW
    bool is_col_child_of_parent2 = false;  // NEW

    std::atomic<bool> is_computed{false};        // have we computed our block
    std::atomic<bool> is_ready_notified{false};  // to have single enqueue
  ...
  class TaskQueue {
   private:
    std::queue<MatrixBlockTask*>
        queue_;         // pointers to ready tasks, SIMPLE FIFO, NOT PRIORITIZED
  ...
  void init(const SampletTreeBase<Derived>& ST, Scalar eta,
    ...
    // 1: BLOCK DISCOVERY
    // We perform a DFS for each column cluster to find all admissible blocks.
    // This creates the initial set of tasks we need to compute. Temporary
    // storage: organize blocks by level
    std::vector<std::unordered_set<size_t>> level_patterns(
        maxlevel);  // FIND A BETTER SOLUTION!!!!!!!!!!!!!
...
#pragma omp parallel for schedule(dynamic)
    for (Index j = 0; j < nclusters; ++j) {
      ...
      while (!row_stack.empty()) {
        ...
        // Push children to stack if they're not admissible (i.e., we need to go
        // deeper)
...
#pragma omp critical
    ...
    for (int level = 0; level < maxlevel; ++level) {
      for (size_t block_key : level_patterns[level]) {
        auto task = std::make_unique<MatrixBlockTask>();  // Create a new task
                                                          // using unique_ptr
    ...
    // 3: DEPENDENCY WIRING
    // For each task, identify its children (dependencies) and wire up the
    // graph. A task depends on children if it needs to recycle their computed
    // blocks.
    ...
    for (auto& [key, task] : task_graph_) {
      ...
      for (auto i = 0; i < pr->nSons(); ++i) {
        ...
        if (auto it = task_graph_.find(child_key); it != task_graph_.end()) {
          // Check if this child block exists in our task graph{
          ...
          if (!child->parent1) {
            ...
            child->is_row_child_of_parent1 = true;
          } else if (!child->parent2) {
            child->parent2 = task.get();
            child->is_row_child_of_parent2 = true;
          }
      ...
      for (auto j = 0; j < pc->nSons(); ++j) {
        ...
        if (auto it = task_graph_.find(child_key); it != task_graph_.end()) {
          ...
          if (!child->parent1) {
            child->parent1 = task.get();
            child->is_col_child_of_parent1 = true;
          } else if (!child->parent2) {
            child->parent2 = task.get();
            child->is_col_child_of_parent2 = true;
          }
    ...
    // Leaf tasks are those with no children - they can be computed immediately
    // without waiting for any dependencies
    leaf_tasks_.clear();
    for (auto& [key, task] : task_graph_) {
  ...
  void compress(const EntGenerator& e_gen) {
    ...
    for (auto* leaf : leaf_tasks_) {
      task_queue_.enqueue(
          leaf);  // NOT PRIORITIZED, ALL LEAVES HAVE EQUAL PRIORITY
    }
    // At this point:
...
#pragma omp parallel
    {
      while (true) {
        ...
        MatrixBlockTask* task =
            task_queue_.dequeue();  // FIFO: First In First Out
 ...
 private:
  ...
  void computeBlock(MatrixBlockTask* task, const EntGenerator& e_gen,
    ...
    switch (the_case) {
      ...
      case 1: {  // CASE 1: (noleaf, leaf) - Row has children, column is leaf
      ...
      case 2: 
      // {  // (leaf, noleaf) - Column has children, row is leaf
      ...
      case 0: {  // (noleaf, noleaf) - can use EITHER row OR column children
        // check if row or column children are ready
        ...
        // Preferenza: usa quello che è pronto per primo
        // Se entrambi pronti, scegli quello con meno children (più efficiente)
        bool use_row_children = false;

        if (row_ready && col_ready) {
          // Entrambi pronti: scegli il più piccolo
          use_col_children =
              (task->total_col_children <= task->total_row_children);
        } else if (row_ready) {
        ...
        if (use_row_children) {
          // Use row children (same logic as case 1)
          task->block.resize(pr->Q().rows(), pc->Q().cols());
          Index offset = 0;
          ...
          // Use column children (same logic as case 2)
          task->block.resize(pr->Q().cols(), pc->Q().rows());
          Index offset = 0;
  ...
  void notifyParents(MatrixBlockTask* task) {
    // Notify first parent
    // Pass relationship flags (pre-computed durante init)
    if (task->parent1) {
      updateParentDependency(task->parent1, task, task->is_row_child_of_parent1,
                             task->is_col_child_of_parent1);
    }

    // Notify second parent (if exists and different from first)
    if (task->parent2 && task->parent2 != task->parent1) {
      updateParentDependency(task->parent2, task, task->is_row_child_of_parent2,
                             task->is_col_child_of_parent2);
    }
  }
  ...
  void updateParentDependency(MatrixBlockTask* parent, MatrixBlockTask* child,
                              bool is_row_child, bool is_col_child) {
    // Update appropriate counter based on relationship type
    if (is_row_child) {
    ...
    // Check if we should enqueue parent
    // CRITICAL: Use exchange to ensure we only enqueue ONCE
    if (!parent->is_ready_notified.exchange(true)) {
      // We're the first to set the flag!
      // Now check if parent is actually ready
      ...
      switch (the_case) {
        ...
        case 1:
          // (noleaf, leaf) - needs ONLY row children
          parent_ready = (row_completed == parent->total_row_children);
          break;

        case 2:
          // (leaf, noleaf) - needs ONLY column children
          parent_ready = (col_completed == parent->total_col_children);
          break;

        case 0:
          // (noleaf, noleaf) - can use EITHER row OR column children
          parent_ready = (row_completed == parent->total_row_children) ||
      ...
      if (parent_ready) {
        ...
        // Parent not actually ready yet, reset flag
