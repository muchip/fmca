...
#pragma omp parallel for
  ...
  template <typename EntGenerator> void compress(const EntGenerator &e_gen) {
    ...
    for (int i = pattern_.size() - 1; i >= 0; --i) {
      ...
      for (auto it = pattern_[i].rbegin(); it != pattern_[i].rend(); ++it) {
        ...
        if (pc->nSons()) {
          for (auto k = 0; k < pc->nSons(); ++k) {
            ...
            const auto it = pattern_[son_id].find(row_id);
            if (it != pattern_[son_id].end()) {
              ++rec_blocks_;
              const Matrix &ret = it->second;
          ...
          if (!pr->nSons()) {
            ...
            for (auto k = 0; k < pr->nSons(); ++k) {
              ...
              const auto it = pattern_[col_id].find(son_id);
              // if so, reuse the matrix block, otherwise recompute it
              if (it != pattern_[col_id].end()) {
                ++rec_blocks_;
                const Matrix &ret = it->second;
