...
#pragma omp critical
 ...
 private:
  ...
  Matrix computeBlock(const Derived &TR, const Derived &TC,
                      const EntryGenerator &e_gen) {
    Matrix buf(TR.Q().rows(), TC.Q().rows());
    Index cur_cols = 0;
    Index cur_rows = 0;
      ...
      switch (the_case) {
        ...
        case 2:
          ...
          for (auto j = 0; j < TC.nSons(); ++j) {
            ...
            buf.middleCols(cur_cols, nscalfs) =
                computeBlock(TR, TC.sons(j), e_gen).leftCols(nscalfs);
            cur_cols += nscalfs;
          }
          return buf * TC.Q();
        case 1:
          ...
          for (auto i = 0; i < TR.nSons(); ++i) {
            ...
            buf.middleRows(cur_rows, nscalfs) =
                computeBlock(TR.sons(i), TC, e_gen).bottomRows(nscalfs);
            cur_rows += nscalfs;
          }
        ...
        case 0:
          ...
          for (auto i = 0; i < TR.nSons(); ++i) {
            Matrix ret1(TR.sons(i).Q().rows(), TC.Q().rows());
            const Index r_nscalfs = TR.sons(i).nscalfs();
            for (auto j = 0; j < TC.nSons(); ++j) {
              const Index c_nscalfs = TC.sons(j).nscalfs();
              ret1.middleCols(cur_cols, c_nscalfs) =
                  computeBlock(TR.sons(i), TC.sons(j), e_gen)
                      .leftCols(c_nscalfs);
              cur_cols += c_nscalfs;
            }
            ret1 = ret1 * TC.Q();
            buf.middleRows(cur_rows, r_nscalfs) = ret1.bottomRows(r_nscalfs);
            cur_rows += r_nscalfs;
          }
          return (buf * TR.Q()).transpose();
      }
