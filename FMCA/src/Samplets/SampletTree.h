 ...
 public:
  ...
  void init(const Eigen::Matrix<value_type, dimension, Eigen::Dynamic> &P,
...
#ifdef USE_QR_CONSTRUCTION_
    {
      ...
      IndexType m_dtilde =
          binomialCoefficient(dimension + dtilde - 1, dimension);
      IndexType dtilde2 = dtilde;
      while (2 * m_dtilde >
             binomialCoefficient(dimension + dtilde2 - 1, dimension))
        ++dtilde2;
      IndexType m_dtilde2 =
          binomialCoefficient(dimension + dtilde2 - 1, dimension);
      std::cout << "ditlde: " << dtilde << " dtilde2: " << dtilde2 << std::endl
                << "mtilde: " << m_dtilde << " mtilde2: " << m_dtilde2
                << std::endl;
      tree_data_->dtilde_ = dtilde;
      tree_data_->dtilde2_ = dtilde2;
      tree_data_->m_dtilde_ = m_dtilde;
      tree_data_->m_dtilde2_ = m_dtilde2;

      tree_data_->idcs.init(tree_data_->dtilde2_ - 1);
      eigen_assert(tree_data_->idcs.get_MultiIndexSet().size() ==
                       tree_data_->m_dtilde2_ &&
...
#endif
    {
      ...
      IndexType i = 0;
      IndexType j = 0;
      tree_data_->multinomial_coefficients.resize(tree_data_->m_dtilde2_,
                                                  tree_data_->m_dtilde2_);
      for (const auto &beta : tree_data_->idcs.get_MultiIndexSet()) {
        for (const auto &alpha : tree_data_->idcs.get_MultiIndexSet()) {
...
#ifdef USE_QR_CONSTRUCTION_
    // are there samplets?
    if (tree_data_->m_dtilde_ < mom_buffer_.cols()) {
      ...
      mom_buffer_ =
          qr.matrixQR()
              .block(0, 0, tree_data_->m_dtilde_, tree_data_->m_dtilde2_)
