...
struct GeometricKDBisection {
  ...
  void operator()(const Matrix &P, std::vector<CTNode> &children,
    ...
    Index num_children = Index(1) << d;
    children.resize(num_children);

    // compute the pivot in each dimension
    std::vector<Scalar> pivots(d);
    for (Index i = 0; i < d; ++i) {
      pivots[i] = parent.bb_(i, 0) + parent.bb_(i, 2) * Scalar(0.5);
    }

    // initialize the children bounding boxes and then modify them
    for (Index child_idx = 0; child_idx < num_children; ++child_idx) {
      ...
      for (Index dim = 0; dim < d; ++dim) {
        ...
        children[child_idx].bb_(dim, 2) *= Scalar(0.5);  // halve bb dimension
        if (upper_half) {
    ...
    for (Index child_idx = 0; child_idx < num_children; ++child_idx) {
      children[child_idx].block_size_ =
          static_cast<Index>(child_indices[child_idx].size());
      ...
      for (Index i = 0; i < children[child_idx].block_size_; ++i) {
        parent_idcs[current_starting_points + i] =
            child_indices[child_idx][static_cast<std::size_t>(i)];
      }
...
struct CoordinateCompare {
  ...
  bool operator()(Index i, Index j) const { return P_(cmp_, i) < P_(cmp_, j); }
};
...
struct ArrayCompare {
  ...
  bool operator()(Index i, Index j) const { return v_(i) < v_(j); }
};
...
struct RandomProjection {
  ...
  void operator()(const Matrix &P, CTNode &c1, CTNode &c2) const {
    ...
    std::vector<Index> loc_idcs(static_cast<std::size_t>(bsize));
    std::iota(loc_idcs.begin(), loc_idcs.end(), Index(0));
    ...
    for (Index i = 0; i < bsize; ++i)
      projections(i) = P.col(idcs[i]).dot(v);

    // sort father index set with respect to the projections array
    {
      std::sort(loc_idcs.begin(), loc_idcs.end(), ArrayCompare(projections));
      std::vector<Index> sorted_indices(static_cast<std::size_t>(bsize));
      for (Index i = 0; i < bsize; ++i)
        sorted_indices[static_cast<std::size_t>(i)] =
            idcs[loc_idcs[static_cast<std::size_t>(i)]];
      std::memcpy(idcs, sorted_indices.data(),
                  static_cast<std::size_t>(bsize) * sizeof(Index));
    }

    // determine splitting point (median assumes that array is never empty)
    const Scalar median = (bsize % 2)
                              ? projections(loc_idcs[static_cast<std::size_t>(
                                    bsize / 2)])
                              : Scalar(0.5) *
                                    (projections(loc_idcs[static_cast<std::size_t>(
                                         bsize / 2)]) +
    ...
    // set delta
    const Scalar delta = Scalar(6.) *
                         (Scalar(2.) * Scalar(std::rand()) /
                              Scalar(RAND_MAX) -
                          Scalar(1.)) *
                         max_dist / sqrtD;
    ...
    Index split_bsize = 0;
    {
      ...
      while (count > 0) {
        ...
        step = count / 2;
        it += step;
        if (projections(loc_idcs[static_cast<std::size_t>(it)]) < medpdelta) {
          split_bsize = ++it;
          count -= step + 1;
        } else {
          count = step;
...
struct FastRandomProjection {
  ...
  void operator()(const Matrix &P, CTNode &c1, CTNode &c2) const {
    ...
    do {
      ...
      // set delta
      const Scalar rdm =
          Scalar(2.) * Scalar(std::rand()) / Scalar(RAND_MAX) - Scalar(1.);
      const Scalar delta = rdm * Scalar(6.) * max_dist / sqrtD;
...
#endif  // FMCA_CLUSTERING_CLUSTERTREESPLITTER_H_
