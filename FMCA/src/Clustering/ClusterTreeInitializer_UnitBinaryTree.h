...
//
#ifndef FMCA_CLUSTERING_INITUNITBINARYTREEIMPL_H_
#define FMCA_CLUSTERING_INITUNITBINARYTREEIMPL_H_
 ...
 **/
template <>
struct ClusterTreeInitializer<UnitBinaryTree> {
  ...
  static void init_ClusterTree_impl(ClusterTreeBase<Derived> &CT,
    ...
    while (queue.size()) {
      ...
      Index axis;
      node.bb().col(2).maxCoeff(&axis);
      const Scalar mid = 0.5 * (node.bb()(axis, 0) + node.bb()(axis, 1));
      for (Index i = 0; i < k; ++i) {
        node.sons(i).node().bb_ = node.node().bb_;
        // set bounding boxes of children
        if (i == 0)
          node.sons(i).node().bb_(axis, 1) = mid;
        else
          node.sons(i).node().bb_(axis, 0) = mid;
