...
/**
 *  \brief these are routines for the case where there is time in the last
 *         coordinate, which is dealt with as being periodic with period
 *         lentght 12 -> 12 month
 *
 **/
template <typename Derived, typename otherDerived>
Scalar computeDistanceTPeriodic(const ClusterTreeBase<Derived> &cluster1,
                                const ClusterTreeBase<otherDerived> &cluster2) {
  const Index n = cluster1.bb().rows();
  const Scalar sinc1 = std::sin(FMCA_PI * cluster1.bb()(n - 1, 2) / 12.);
  const Scalar sinc2 = std::sin(FMCA_PI * cluster2.bb()(n - 1, 2) / 12.);
...
Admissibility compareClusterTPeriodic(
  ...
  const Scalar dist = computeDistanceTPeriodic(cluster1, cluster2);
  const Scalar row_radius = 0.5 * cluster1.bb().col(2).norm();
  const Scalar col_radius = 0.5 * cluster2.bb().col(2).norm();
  const Scalar radius = row_radius > col_radius ? row_radius : col_radius;
