...
int main(int argc, char *argv[]) {
  ...
  FMCA::Matrix Qpts;
  {
    FMCA::Matrix inputPts = readMatrix("../Points/bunnyFine.txt");
    FMCA::Matrix rotation = FMCA::Matrix::Random(dim, dim);
    {
      Eigen::HouseholderQR<FMCA::Matrix> qr;
      qr.compute(rotation);
      rotation = qr.householderQ();
    }
    std::cout << rotation << std::endl;
    Qpts.resize(dim, inputPts.rows());
    Qpts.topRows(3) = inputPts.transpose();
    Qpts = rotation * Qpts;
    std::cout << "domain diam: "
              << (Qpts.rowwise().maxCoeff() - Qpts.rowwise().minCoeff()).norm()
              << std::endl;
  }
  FMCA::Tictoc T;
  for (FMCA::Index npts : {1e3, 5e3, 1e4, 5e4, 1e5, 5e5, 1e6, 2e6}) {
    ...
    T.tic();
    const Eigen::MatrixXd P = Qpts.leftCols(npts);
