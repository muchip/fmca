...
int main(int argc, char *argv[]) {
  ...
  const double tcomp = T.toc("cummulative compressor:     ");
  output_file << max_min_dist << " \t" << min_min_dist << " \t" << tcomp
              << " \t";
  ...
  double lambda_max = 0;
  {
    ...
    std::cout << "lambda_max (20power its):    " << lambda_max << std::endl;
  }
  ...
  output_file << 100. * inv_trips.size() / npts / npts << " \t";
  {
    ...
    std::cout << "inverse error:               "
              << (rand - Rrand).norm() / rand.norm() << std::endl;
