...
#include "../FMCA/src/Samplets/samplet_matrix_compressor_DAG.h"
...
int main() {
  ...
  for (int dtilde = 3; dtilde <= 3; ++dtilde) {
    ...
    for (auto i = 0; i < 10; ++i) {
      ...
      x.setZero();
      x(index) = 1;
      FMCA::Vector col = function.eval(P, P.col(st.indices()[index]));
      y1 =
          col(Eigen::Map<const FMCA::iVector>(st.indices(), st.block_size()));
