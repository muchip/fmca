...
//////////////////////////////////////////////////////////////////////////////////////////
int main() {
  constexpr FMCA::Index l = 10;
  constexpr FMCA::Index d = 2;
  constexpr FMCA::Index N = 1 << l;
  FMCA::Index Nd = std::pow(N, d);
  constexpr FMCA::Index dtilde = 3;
  constexpr FMCA::Scalar lambda = 2.5; // Rate parameter for exponential distribution
  ...
  T.tic();
  {
    ...
    std::exponential_distribution<FMCA::Scalar> exp_dist(lambda);
    
    for (FMCA::Index i = 0; i < Nd; ++i) {
      for (FMCA::Index dim = 0; dim < d; ++dim) {
        // Generate exponential random variable and normalize to [0,1]
        FMCA::Scalar exp_val = exp_dist(gen);
        // Use CDF transformation: 1 - exp(-lambda * x) to map to [0,1]
        // Invert: x = -log(1 - u) / lambda, where u ~ Uniform[0,1]
        // But we already have exponential, so we normalize differently
        P(dim, i) = std::min(exp_val / (5.0 / lambda), 1.0); // Scale to [0,1]
      }
    }
    
    // Alternative: True exponential CDF mapping
    std::uniform_real_distribution<FMCA::Scalar> unif(0.0, 1.0);
    for (FMCA::Index i = 0; i < Nd; ++i) {
      for (FMCA::Index dim = 0; dim < d; ++dim) {
        FMCA::Scalar u = unif(gen);
        // CDF^{-1}: x = -log(1-u)/lambda, normalize to [0,1]
        P(dim, i) = -std::log(1.0 - u * (1.0 - std::exp(-lambda))) / lambda;
      }
    }
  }
  {
  ...
  P3.topRows(2) = P;
  P3.row(2).setZero();
  FMCA::IO::plotPointsColor("Slope2D_Exponential.vtk", P3, colr);
  FMCA::IO::plotPointsColor("Function2D_Exponential.vtk", P3, data);

  Matrix P3_values(3, P.cols());
  P3_values.topRows(2) = P;
  P3_values.row(2) = data;
  FMCA::IO::plotPointsColor("Function2D_3D_Exponential.vtk", P3_values, data);
