...
void runMultigridTest(Scalar nu, Scalar adaptive_threshold = 1e-3,
  ...
  for (int l = 0; l < max_level; ++l) {
    ...
    if (l + 1 < max_level) {
      ...
      // Add contributions from all previous levels
      for (int j = 0; j <= l; ++j) {
        Scalar sigma_j = nu * fill_distances[j];
        CovarianceKernel kernel_j(kernel_type, sigma_j);
        MultipoleFunctionEvaluator evaluator;
      ...
      Index counter_new_points = 0;

      std::cout << "Initial max error: " << error << std::endl;
      std::cout << "f-greedy threshold * delta: " << f_greedy_threshold * sigma_l << std::endl;

      // Greedy loop: add points until error threshold is met
      while (error > f_greedy_threshold / sigma_l ) { // || counter_new_points < n_candidates/4
        Index max_idx;
        errors.maxCoeff(&max_idx);
        ...
        if (counter_new_points > 1) {
          P_next_new.leftCols(counter_new_points - 1) = P_next;
        }
        P_next_new.col(counter_new_points - 1) = P_candidates.col(max_idx);
        P_next = P_next_new;

        // Mark this point as selected (set error to zero)
        errors(max_idx) = 0;

        // Update error for remaining points
        error = errors.maxCoeff();

        // Print progress periodically
        // if (counter_new_points % 100 == 0 || counter_new_points < 10) {
      ...
      std::cout << "- Selected points: " << counter_new_points << " / "
                << n_candidates << " ("
      ...
      plot_filename << "active_points_fgreedy_level_" << (l + 1) << ".vtk";
  ...
  for (size_t i = 0; i < levels.size(); ++i) {
    ...
    std::cout << std::left << std::setw(10) << levels[i] << std::setw(10)
              ...
              << cg_time[i] << std::setw(10) << iterationsCG[i]
              << std::setw(10) << static_cast<int>(anz[i]) << std::setw(15)
              << std::scientific << std::setprecision(6) << l2_errors[i]
              << std::setw(15) << std::scientific << std::setprecision(6)
              << linf_errors[i] << std::endl;
  }
}
