...
#pragma omp critical
  ...
  std::cout << "inverse error vs H-Matrix:    " << (Y2 - X).norm() / X.norm()
            << std::endl;
  T.tic();
  FMCA::Matrix HK = hmat.full();
  FMCA::Matrix invHK = HK.inverse();
  FMCA::Matrix invHKoord = invHK;
  ...
  T.toc("inverted full H matrix / rec:");
  {
    ...
    Y2 = invhmat * X;
    FMCA::Scalar err = (Y1 - Y2).norm() / Y1.norm();
    std::cout << "compression error invH:       " << err << std::endl;
    std::cout << std::string(60, '-') << std::endl;
  }
  ...
  Y1 = hmat * X;
  Y2 = invhmat * Y1;
  std::cout << "H inverse error:              " << (Y2 - X).norm() / X.norm()
            << std::endl;
