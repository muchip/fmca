...
using EigenCholesky =
    Eigen::SimplicialLLT<largeSparse>;
...
int main() {
  ...
  largeSparse Wcomp_border_largeSparse(NPTS_QUAD_BORDER, NPTS_QUAD_BORDER);
  Wcomp_border_largeSparse.setFromTriplets(trips_weights_border.begin(),
                                           trips_weights_border.end());
  Wcomp_border_largeSparse.makeCompressed();
  //////////////////////////////////////////////////////////////////////////////
  ...
  largeSparse Kcomp_largeSparse(NPTS_SOURCE, NPTS_QUAD);
  Kcomp_largeSparse.setFromTriplets(trips.begin(), trips.end());
  Kcomp_largeSparse.makeCompressed();
  //////////////////////////////////////////////////////////////////////////////
  ...
  largeSparse Kcomp_sources_quadborder_largeSparse(NPTS_SOURCE,
                                                   NPTS_QUAD_BORDER);
  Kcomp_sources_quadborder_largeSparse.setFromTriplets(
      trips_sources_quadborder.begin(), trips_sources_quadborder.end());
  Kcomp_sources_quadborder_largeSparse.makeCompressed();
  ...
  largeSparse Kcomp_border_largeSparse(NPTS_SOURCE_BORDER, NPTS_QUAD_BORDER);
  Kcomp_border_largeSparse.setFromTriplets(trips_border.begin(),
                                           trips_border.end());
  Kcomp_border_largeSparse.makeCompressed();
  //////////////////////////////////////////////////////////////////////////////
  // create A
  T.tic();
  FMCA::Matrix A = Kcomp_sources_quadborder_largeSparse *
                  (Wcomp_border_largeSparse.selfadjointView<Eigen::Upper>() *
                   Kcomp_border_largeSparse.transpose())
                      .eval();
  double mult_time_eigen_A = T.toc();
  std::cout << "eigen multiplication time matrix A:    " << mult_time_eigen_A
            << std::endl;
  //////////////////////////////////////////////////////////////////////////////
  FMCA::Matrix stiffness(NPTS_SOURCE, NPTS_SOURCE);
  stiffness.setZero();
  ...
  for (int i = 0; i < DIM; ++i) {
    ...
    T.tic();
    FMCA::Matrix res_eigen =
        Scomp_largeSparse * (Wcomp_largeSparse.selfadjointView<Eigen::Upper>() *
                             Scomp_largeSparse.transpose())
                                .eval();
  ...
  FMCA::Vector F_comp =
      (Kcomp_largeSparse * Wcomp_largeSparse.selfadjointView<Eigen::Upper>()) *
      (hst_quad.sampletTransform(6 * f));
  std::cout << "F_comp created" << std::endl;
  //////////////////////////////////////////////////////////////////////////////
  // G_comp = right hand side 2
  FMCA::Vector G_comp =
      (Kcomp_border_largeSparse *
       Wcomp_border_largeSparse.selfadjointView<Eigen::Upper>()) *
  ...
  std::cout << "stiffness - stiffnessT: "
            << (stiffness - stiffness.transpose()).norm() / stiffness.norm()
            << std::endl;
  std::cout << "Frobenius norm A: " << A.norm() << std::endl;
  //////////////////////////////////////////////////////////////////////////////
  // solve
  // FMCA::Vector numerical_sol = UzawaAlgorithm(stiffness.sparseView(), A.sparseView(), F_comp, G_comp);
  ...
  FMCA::Vector solution = systemMatrix.fullPivLu().solve(rhs);
  FMCA::Vector u = solution.head(stiffness.rows());
  FMCA::Vector lambda = solution.tail(A.cols());
