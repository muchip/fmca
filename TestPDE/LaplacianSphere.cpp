...
int main() {
  ...
  readTXT("data/Points_Sphere_Uniform.txt", P, DIM);
  ...
  std::vector<double> sigmas = {0.5, 0.8, 1, 1.5, 2};
  ...
  for (FMCA::Scalar sigma_factor : sigmas) {
    ...
    FMCA::Vector u = SolvePoisson_constantWeights(
        DIM, P, P_interior, w_vec, P_bnd, w_vec_border, Normals, u_bc, f, sigma, eta,
        dtilde, threshold_kernel, threshold_gradKernel, threshold_weights,
