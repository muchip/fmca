...
/*
  ...
  saveMatrixToFile(P_interior.transpose(),"MC_Interior_Sphere_collocation.txt");
  int N_bnd = 50000;
  FMCA::Matrix P_bnd = MonteCarloPointsBoundary(N_bnd);
  saveMatrixToFile(P_bnd.transpose(), "MC_Bnd_Sphere_collocation.txt");
*/
...
int main() {
  ...
  const FMCA::Scalar threshold_filtered_triplets = 1e-4;
  const FMCA::Scalar number_points_perBall = 5;
  ...
  for (double sigma : sigma_values) {
    ...
    std::cout << "threshold kernel            " << threshold_kernel
              << std::endl;
    ...
    for (int i = 0; i < DIM; ++i) {
      ...
      FMCA::internal::SampletMatrixCompressorUnsymmetric<H2SampletTree>
          Laplacian_Interior;
      Laplacian_Interior.init(hst_interior, hst, eta,
    ...
    for (int k = 0; k < A.outerSize(); ++k) {
      for (Eigen::SparseMatrix<double>::InnerIterator it(A, k); it; ++it) {
        if (it.row() == it.col()) {
          filteredTriplets.push_back(
              Eigen::Triplet<double>(it.row(), it.col(), it.value()));
        } else {
          if (std::abs(it.value()) >= threshold_filtered_triplets) {
            filteredTriplets.push_back(
                Eigen::Triplet<double>(it.row(), it.col(), it.value()));
          }
        }
    ...
    A_filtered.setFromTriplets(filteredTriplets.begin(),
                               filteredTriplets.end());
    A_filtered.makeCompressed();
    std::cout << "anz Matrix system filtered     "
              << filteredTriplets.size() / N << std::endl;
    std::cout << "Matrix - Matrix filtered       "
