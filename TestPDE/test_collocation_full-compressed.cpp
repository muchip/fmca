...
FMCA::Vector generateNormalVector(int n) {
  ...
  std::normal_distribution<> d(
      0, 1);  // Normal distribution with mean 0 and std dev 1
  for (int i = 0; i < n; ++i) {
    X[i] = d(gen);
  }
  return X / X.norm();
...
int main() {
  ...
  //////////////////////////////////////////////////////////////////////////////
  std::cout << std::string(80, '-') << std::endl;
  std::cout << "Number of interior points           " << NPTS_INTERIOR
            << std::endl;
  std::cout << "Number of border points             " << NPTS_BORDER
            << std::endl;
  std::cout << "Total number of points              " << N << std::endl;
  std::cout << "sigma                               " << sigma << std::endl;
  //////////////////////////////////////////////////////////////////////////////
  // Kernel border compressed
  ...
  FMCA::Matrix K_bnd_n_eval_permuted =
      Perm_bnd.transpose() * K_bnd_n_eval * Perm_N;
  //////////////////////////////////////////////////////////////////////////////
  // Laplacian
  ...
  for (int i = 0; i < DIM; ++i) {
    ...
    FMCA::Matrix Laplacian_Interior_eval_permuted =
        Perm_int.transpose() * Laplacian_Interior_eval * Perm_N;
  ...
  std::cout << "anz A:                          " << tripletList.size() / N
            << std::endl;

  ///// error compressed - not compressed
  ...
  FMCA::Matrix TBT_eval =
      hst_sources_border.sampletTransform(K_bnd_n_eval_permuted);
  TBT_eval = hst.sampletTransform(TBT_eval.transpose()).transpose();
  std::cout << "Laplacian error eval                    "
            << (Laplacian.toDense() - TLT_eval).norm() / TLT_eval.norm()
            << std::endl;
  std::cout << "Border error eval                       "
            << (Kcomp_border.toDense() - TBT_eval).norm() / TBT_eval.norm()
            << std::endl;
  FMCA::Matrix A_eval_samplets(N, N);
  A_eval_samplets << TLT_eval, TBT_eval;
  std::cout << "Matrix system error                     "
            << (A_eval_samplets - A.toDense()).norm() / A_eval_samplets.norm()
            << std::endl;
  //////////////////////////////////////////////////////////////////////////////
  ...
  FMCA::Vector alpha_eval = solver_eval.solve(b_eval);
  ...
  std::cout << "K * alpha_dense - K * alpha_compressed     "
            << (K_eval_n * alpha_eval - K_eval_n * alpha_permuted).norm() /
                   (K_eval_n * alpha_eval).norm()
            << std::endl;
  ...
  std::cout << "error compressed    "
            << (analytical_sol - K_eval_n * alpha_permuted).norm() /
                   (K_eval_n * alpha_permuted).norm()
            << std::endl;

  std::cout << "error full          "
            << (analytical_sol - K_eval_n * alpha_eval).norm() /
                   (K_eval_n * alpha_eval).norm()
            << std::endl;
