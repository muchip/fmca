...
int main() {
  ...
  for (FMCA::Scalar sigma : sigmas) {
    ...
    applyPatternAndFilter(Matrix_system_half, a_priori_triplets, 1e-4);
    ...
    std::cout << "solver time chol:                                  "
              << solver_time_chol << std::endl;

    Eigen::CholmodSupernodalLLT<Eigen::SparseMatrix<double>> cholmod;
    cholmod.compute(Matrix_system);
    T.tic();
    FMCA::Vector u_cholmod = cholmod.solve(F_comp + beta * G_comp);
    FMCA::Scalar solver_time_cholmod = T.toc();
    std::cout << "solver time cholmod:                                  "
              << solver_time_cholmod << std::endl;

    std::cout << "residual error chol:                                  "
              << ((Matrix_system)*u - (F_comp + beta * G_comp)).norm()
              << std::endl;
    std::cout << "residual error cholmod:                                  "
              << ((Matrix_system)*u_cholmod - (F_comp + beta * G_comp))
                     .norm()
              << std::endl;
    ...
    // FMCA::Scalar error = (KU - analytical_sol).norm() /
    analytical_sol.norm();
    // std::cout << "Error:                                            " <<
    error
    //           << std::endl;
