...
int main() {
  ...
  FMCA::Matrix P;
  //////////////////////////////////////////////////////////////////////////////
  // readTXT("data/vertices_interior_cube.txt", P_sources, DIM);
  // readTXT("data/vertices_boundary_cube.txt", P_sources_border, DIM);
  // readTXT("data/Int_and_Bnd_cube.txt", P, DIM);
  readTXT("data/vertices_interior_bunny.txt", P_sources, DIM);
  readTXT("data/vertices_boundary_bunny.txt", P_sources_border, DIM);
  readTXT("data/Int_and_Bnd_bunny.txt", P, DIM);
  ...
  std::vector<double> sigma_values = {0.05};
  std::string kernel_type = "MATERN52";
  std::string kernel_type_laplacian = "MATERN52_SECOND_DERIVATIVE";
  std::string spascii = "results_matlab/collocation3d";
  std::string filename = "collocation3d";
  ...
  for (double sigma : sigma_values) {
    ...
    FMCA::IO::print2m(
        "results_matlab/solution_collocation_compressed3d_SampletBasis.m",
        "sol_collocation_compressed3d_SampletBasis", alpha, "w");
    ...
    FMCA::IO::print2m("results_matlab/solution_collocation_compressed3d.m",
                      "sol_collocation_compressed3d", Kalpha_permuted, "w");
    // FMCA::IO::print2ascii("sol_collocation_compressed", Kalpha_permuted);
  }
  return 0;
}
