...
Scalar DistanceCenterofMass(Scalar x, Scalar y, Scalar z, Scalar x_m,
  ...
  return std::sqrt((x - x_m) * (x - x_m) + (y - y_m) * (y - y_m) +
                   (z - z_m) * (z - z_m));
}
...
Vector evalFunction(const Matrix& Points, Scalar x_m, Scalar y_m, Scalar z_m) {
  ...
  for (Eigen::Index i = 0; i < Points.cols(); ++i) {
    f(i) = DistanceCenterofMass(Points(0, i), Points(1, i), Points(2, i), x_m,
                                y_m, z_m);
  }
  return f;
}
...
int main() {
  ...
  readTXT("data/armadillo_level1.txt", P1, DIM);
  readTXT("data/armadillo_level2.txt", P2, DIM);
  readTXT("data/armadillo_level3.txt", P3, DIM);
  readTXT("data/armadillo_level4.txt", P4, DIM);
  readTXT("data/armadillo_level5.txt", P5, DIM);
  readTXT("data/armadillo_level6.txt", P6, DIM);
  ...
  Matrix Peval;
  readTXT("data/armadillo_eval.txt", Peval, DIM);
  ...
  for (Eigen::Index i = 0; i < max_level; ++i) {
    ...
    std::cout << "norm residual " << i << " " << residual.norm() << std::endl;
  }
  std::cout << fill_distances << std::endl;

  ///////////////////////////////// Coeffs Initialization
  std::vector<Vector> ALPHA;
  std::string base_filename_residuals = "Plots/ResidualsArmadillo";
  ...
  for (Eigen::Index l = 0; l < max_level; ++l) {
    ...
    ///////////////////////////////// Plot the residuals
    {
      ...
      Plotter3D plotter;
      plotter.plotFunction(filename_residuals, P_Matrices[l], residual_natural_basis);
    }
  ...
  Vector solution = Evaluate(
      ...
      dtilde, exact_sol, hst, "Plots/SolutionArmadillo"); // Plots/SolutionBunny
