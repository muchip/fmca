...
void runForMu(Scalar nu) {
  ...
  filename << "ResultsLucy_nu" << nu << "_matern12_stats3.txt";
  ...
  for (int i = 0; i < max_level; ++i) {
    ...
    Vector resid = evalFunction(P_Matrices[i], x_m, y_m, z_m);
  ...
  std::vector<double>
      anz;  // #triplets_apriori / n_points for the symmetric block
  ...
  for (int l = 0; l < max_level; ++l) {
    ...
    for (int j = 0; j < l; ++j) {
      ...
      CompressionResult crB = UnsymmetricCompressorWithStats(
          mom, samp_mom, hst, kernel_B, eta, threshold_kernel, threshold_aPost,
          mpole_deg, dtilde, P_Matrices[l], P_Matrices[j]);

      // sum the times for unsymmetric
      double sumBlock = crB.stats.time_planner + crB.stats.time_compressor +
    ...
    CompressionResult crA = SymmetricCompressorWithStats(
        mom, samp_mom, hst, kernel_l, eta, threshold_kernel, threshold_aPost,
        mpole_deg, dtilde, P_Matrices[l]);

    double sumSymm = crA.stats.time_planner + crA.stats.time_compressor +
    ...
    SolverResult sres =
        solveSystemWithStats(crA.matrix, rhs, "ConjugateGradientwithPreconditioner", 1e-6);
    // store alpha
