...
void runForMu(Scalar nu) {
  ...
  filename << "ResultsLShape_nu" << nu << "_matern12_stats3.txt";
  ...
  for (int i = 0; i < max_level; ++i) {
    ...
    Vector resid = evalAnalyticalSolution(P_Matrices[i]);
  ...
  for (int l = 0; l < max_level; ++l) {
    ...
    SolverResult sres =
        solveSystemWithStats(crA.matrix, rhs, "ConjugateGradient", 1e-6);
    // store alpha
